...
const captainsDb = [
    ...
    { id: 'thorvald', name: 'Торвальд "Седая Сталь"', desc: 'Всякий раз, когда ваш драконьер НЕ поймал дракона, его сила растет на +2 (вместо +1).', phase: 'passive' },
    { id: 'bramm', name: 'Брамм "Мёртвый узел"', desc: 'Один раз в раунд в фазу засады сбросьте 1 своего дракона, чтобы дать +X силы невыставленному диску (X = короны дракона).', phase: 'ambush_active' },
    { id: 'sylvia', name: 'Сильвия "Леди Ночь"', desc: 'Вы начинаете игру с силой 2, 3 и 4. Один раз в фазу засады вы можете подсмотреть любой скрытый диск на поле.', phase: 'ambush_active' },
    { id: 'quint', name: 'Квинт "Золотой Змей"', desc: 'В фазу засады пожертвуйте 1 силы невыставленного диска, чтобы мгновенно выполнить контракт, для которого вам не хватает ровно 1 карты.', phase: 'ambush_active' }
];
...
const treasuresDb = [
    ...
    { type: 'treasure', name: "Белое яйцо", short: "🥚/⚪", desc: "Считается яйцом и сокр. Можно перекрасить", isEgg: true, color: 'white', isOriginallyWhite: true },
...
function getLiveScores() {
    ...
    if (majC && maxMajCount > 0) {
        for (let id in results) if (results[id].majCount === maxMajCount) results[id].contPts += majC.points;
    }
...
function renderInventories() {
    ...
    for (let [id, p] of Object.entries(gameState.players)) {
        ...
        html += `<div class="inv-panel inv-${id}"><div><b style="font-size:18px;">${p.name}</b> ${scoreDisplay}<br><span style="color:#bdc3c7; font-size:12px;">Сила: ${p.powers.join(', ')}</span><br>${capHtml}</div><div class="trophies-container">${stacksHtml}<div class="treasure-stack">${trHtml}</div></div></div>`;
    }
    ...
    whites.forEach(w => {
        ...
        for (let c of gameState.contracts) {
            if (!c.winner && c.type !== 'majority') {
                if (gameState.players[team].trophies.filter(t => t.color === c.color).length + 1 >= c.req) { chosenColor = c.color; break; }
            }
        }
...
function botTurn(team) {
    ...
    if(cap === 'bramm' && !usedCap) {
        ...
        if(weakDragIdx > -1) {
            ...
            gameState.captainsUsedRound[team] = true; logMsg(`🤖 Брамм (${team}) сбросил дракона ради силы!`);
        }
    }
    ...
    emptySlots.forEach(slot => {
        availIndices.forEach(dIndex => {
            ...
            dragons.forEach(d => {
                ...
                if (power >= dPower) {
                    score += 4; 
                    gameState.contracts.forEach(c => { if (!c.winner && c.color === d.dataset.color) score += 10; });
                } else score += 2; 
            });
    ...
    if (bonus && bonus.startsWith('loot') && slotToLoot.dataset.obj) {
        ...
        let loc = slotToLoot.closest('.location'); let d = Array.from(loc.querySelectorAll('.dragon-card'));
            ...
            for (let slot of slots) {
                ...
                if (disk && disk.style.opacity !== '0') {
                    ...
                    if (hPower >= dPower) {
                        ...
                        if (isOrigWhite && gameState.players[hTeam].isBot && cap !== 'eleonora') botRecolorWhite(hTeam); 
                        ...
                        if(cap === 'thorvald') {
                            ...
                            gameState.players[hTeam].powers[hIndex]++; logMsg(`❌ Змей ослаблен до ${dPower}. Охотник получает +1 к силе!`, "log-fail");
                        }
