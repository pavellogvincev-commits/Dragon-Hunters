// === ФАЙЛ: ui.js (Отрисовка, Модалки и Подсчет Очков) ===
...
window.applyUpgrade = function(index) {
    ...
    renderInventories(); if (upgradeResolve) upgradeResolve();
};
window.cancelUpgrade = function() { 
    document.getElementById('upgrade-modal').style.display = 'none'; if (upgradeResolve) upgradeResolve(); 
};
...
window.openColorModal = function(index) { if (gameState.isActionPaused) return; pendingWhiteIndex = index; document.getElementById('color-modal').style.display = 'block'; };
